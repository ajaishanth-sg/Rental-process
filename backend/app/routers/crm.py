...
class PublicLeadSubmission(BaseModel):
    ...
    desiredStartDate: Optional[str] = None
    location: Optional[str] = None
...
async def create_public_lead(lead_data: PublicLeadSubmission):
    """Accept public website enquiries without requiring authentication"""
    try:
        db = get_database()

        now = datetime.now()

        # Locate a default sales representative to own the enquiry
        assigned_salesperson = await db.users.find_one({"role": "sales"})
        ...
        lead_doc = {
            ...
            "industry": lead_data.equipmentCategory,
            "source": "Website",
            "status": "New",
            "leadOwner": assigned_salesperson_name or "Sales Team",
        ...
        enquiry_doc = {
            ...
            "customer_id": f"public-{lead_id}",
            "customer_name": full_name or lead_data.email,
...
async def get_assigned_leads(current_user: dict = Depends(get_current_user)):
    """Return leads assigned to the current salesperson (or all leads for admin)"""
    try:
        if current_user.get("role") not in ["sales", "admin"]:
            raise HTTPException(status_code=403, detail="Only sales or admin users can access assigned leads")
        ...
        if current_user.get("role") == "sales":
            query = {
                "$or": [
                    {"assigned_salesperson_id": current_user.get("id")},
...
async def update_lead_status(lead_id: str, status_update: LeadStatusUpdate, current_user: dict = Depends(get_current_user)):
    """Allow sales or admin users to update lead pipeline status"""
    try:
        if current_user.get("role") not in ["sales", "admin"]:
            raise HTTPException(status_code=403, detail="Only sales or admin users can update lead status")
        ...
        if status_update.nextFollowUp:
            updates["nextFollowUp"] = status_update.nextFollowUp

        if current_user.get("role") == "sales":
            updates["assigned_salesperson_id"] = current_user.get("id")
            updates["assigned_salesperson_name"] = current_user.get("name")

        activity_entry = {
            "type": "status_update",
            "description": f"Lead status updated to {status_update.status}",
            "by": current_user.get("name"),
            "timestamp": datetime.utcnow().isoformat(),
        }

        if status_update.notes:
            activity_entry["notes"] = status_update.notes
        ...
        result = await db.leads.update_one({"lead_id": lead_id}, update_operations)

        if result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Lead status update failed")
